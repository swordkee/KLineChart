...
export interface Store {
  ...
  getFixedVisibleRange: () => Nullable<{ from: number; to: number }>
  resetData: () => void
}
...
export default class StoreImp implements Store {
  ...
  private readonly _formatter = {
    formatDate: ({ dateTimeFormat, timestamp, template }: FormatDateParams) => formatTimestampByTemplate(dateTimeFormat, timestamp, template),
    formatBigNumber,
  ...
  private _addData(data: KLineData | KLineData[], type: DataLoadType, more?: DataLoadMore): void {
    let success = false
  ...
  getFixedVisibleRange(): Nullable<{ from: number; to: number }> {
    return this._fixedVisibleRange ? { ...this._fixedVisibleRange } : null
  }

  setCrosshair(crosshair?: Nullable<Crosshair>, options?: { notInvalidate?: boolean; notExecuteAction?: boolean; forceInvalidate?: boolean }): void {
  ...
  setHoverOverlayInfo(info: EventOverlayInfo, processOnMouseEnterEvent: ProcessOverlayEventCallback, processOnMouseLeaveEvent: ProcessOverlayEventCallback): void {
  ...
  setClickOverlayInfo(info: EventOverlayInfo, processOnSelectedEvent: ProcessOverlayEventCallback, processOnDeselectedEvent: ProcessOverlayEventCallback): void {
